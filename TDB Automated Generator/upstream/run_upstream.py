...
def discover_sqs_dirs(phase_root: Path) -> List[Path]:
    ...
    for d in sorted(phase_root.rglob("*")):
        if d.is_dir() and (d / "str.out").is_file() \
            ...
            # refine_skip: generated by the fit-error refiner but NOT
            # selected — the fit does not need this composition, so
            # don't burn VASP time on it (refine.select_new_dirs).
            if (d / "refine_skip").is_file():
                continue
            out.append(d)
...
def process_phase(phase: str,
                  ...
                  parallel_sqs: bool = False,
                  svib_linear_tol: float = 0.1) -> Dict:
    print(f"\n{'='*70}\n  PHASE {phase}\n{'='*70}")
    ...
    def _one(d: Path) -> Dict:
        ...
        # adaptive scope (2026-07-22): phonons at endmembers AND lev=1
        # up front — the lev=1 point is what tests the linear-svib
        # assumption; lev=2 phonons are bought only if it fails
        # (refine.adaptive_svib_phase below).
        in_scope = is_endmem or (phonon_scope == "adaptive"
                                 and "lev=1" in d.name)
        skip_ph = skip_phonon or (phonon_scope in ("endmembers",
                                                   "adaptive")
                                  and not in_scope)
        if skip_ph and not skip_phonon:
            print(f"    (phonons: {phonon_scope} scope — skipped for "
                  f"this mixing SQS)")
    ...
    # Adaptive svib (2026-07-22 user spec): test the linear-svib
    # assumption against the lev=1 phonons; buy lev=2 phonons only
    # where the test fails (more-stable side first, other side only if
    # the 4-point quadratic fits worse than the lev=1 prediction).
    if phonon_scope == "adaptive" and not skip_phonon and results:
        import refine
        enc = preset[0] if preset else results[0]["chosen_encut"]
        kpp = preset[1] if preset else results[0]["chosen_kppra"]
        ph_encut = enc
        if potcar_paths:
            ph_encut = potcar.pulay_safe_encut(
                enc, potcar.max_enmax(potcar_paths))

        def _buy_phonon(d: Path) -> None:
            stamp(f"[{d.name}] adaptive-svib lev=2 phonons starting")
            phonon.run_fitfc(
                d, encut=ph_encut, kppra=kpp, dlm=dlm, algo=algo,
                env_bin=env_bin, timeout=timeout,
                cmd_prefix=cmd_prefix, **(fitfc_opts or {}))

        x_el = sigma_elements[1] if len(sigma_elements) > 1 else \
            sigma_elements[0]
        svib_out = refine.adaptive_svib_phase(
            phase_root, element=x_el, run_phonon=_buy_phonon,
            tol=svib_linear_tol, log=lambda m: stamp(m.strip()))
        stamp(f"[{phase}] svib-adaptive: {svib_out.get('decision')}")
        return {"phase": phase, "sqs": results,
                "preset_out": list(preset) if preset else None,
                "svib_adaptive": svib_out}
...
def main():
    ...
    ap.add_argument("--phonon-scope",
                    choices=("endmembers", "all", "adaptive"),
                    default="endmembers",
                    help="'endmembers' (default, the published sqs2tdb "
                         ...
                         "compute). 'adaptive' (2026-07-22): endmembers "
                         "+ lev=1, then TEST the linear-svib assumption "
                         "— if refuted, buy lev=2 phonons on the more-"
                         "stable side and fit a quadratic, falling back "
                         "to the other side only when the 4-point RMSE "
                         "beats the lev=1 prediction error (decisions "
                         "in <phase>/svib_adaptive.json).")
    ap.add_argument("--svib-linear-tol", type=float, default=0.1,
                    help="adaptive scope: |svib(lev=1) - linear "
                         "prediction| (k_B/atom) above which linearity "
                         "counts as refuted (default 0.1).")
    ...
    for phase in phases:
        res = process_phase(
            ...
            svib_linear_tol=args.svib_linear_tol)
